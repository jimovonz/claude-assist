...
function renderActionHtml(action: ViewAction): string {
  ...
  switch (action.type) {
    case "button":
      return `<div class="action-group action-buttons" data-action-id="${eid}">
        <button type="button" class="action-btn" data-action-id="${eid}" onclick="doButton(this)">${escapeHtml(action.label)}</button>
      </div>`;
    ...
    case "text":
      return `<div class="action-group" data-action-id="${eid}">
        <div class="action-label">${escapeHtml(action.label)}</div>
        <input type="text" class="action-input" name="action-${eid}" placeholder="${escapeHtml(action.placeholder ?? "")}" />
      </div>`;
...
<body>
  ...
  <div class="actions" id="action-bar">
    <form id="action-form" onsubmit="return submitForm(event)">
      ${actions.map(a => renderActionHtml(a)).join("\n      ")}
      <div class="action-form-footer">
        <button type="submit" class="action-btn action-primary" id="submit-btn">Submit</button>
  ...
  <script>
    ...
    // Standalone button — immediate action, closes form
    async function doButton(btn) {
      ...
      try {
        await fetch('/api/action', {
          ...
          body: JSON.stringify({ viewId: VIEW_ID, actionId, label }),
        });
    ...
    // Form submit — collects all inputs, one POST, closes form
    async function submitForm(e) {
      ...
      document.querySelectorAll('.action-group').forEach(group => {
        ...
        if (radio) { data[actionId] = radio.value; return; }

        // Checkboxes
        const checks = Array.from(group.querySelectorAll('input[type="checkbox"]:checked')).map(i => i.value);
        if (checks.length) { data[actionId] = checks.join(', '); return; }

        // Text
        const text = group.querySelector('input[type="text"]');
        if (text && text.value.trim()) { data[actionId] = text.value.trim(); return; }
      });
      ...
      try {
        const label = Object.entries(data).map(([k,v]) => k + ': ' + v).join('; ');
        await fetch('/api/action', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ viewId: VIEW_ID, actionId: 'form-submit', label, value: JSON.stringify(data) }),
        });
        bar.classList.add('submitted');
      } catch (err) {
        btn.disabled = false;
